...
GameOverWidget::GameOverWidget(QWidget *parent) : QWidget(parent) {
  ...
  lowerHalf->layout()->addItem(
      new QSpacerItem(0, 0, QSizePolicy::Expanding, QSizePolicy::Minimum));
  lowerHalf->layout()->addWidget(scoreBoard);
  scoreLabel->setAlignment(Qt::AlignCenter);
  scoreLabel->setStyleSheet(
      "QLabel {color: white; font-size: 30px; font-weight: bold; }");
  return;
  scoreBoard->setHorizontalHeaderLabels(QStringList() << "Name"
                                                      << "Score");
  tableItem t[] = {{"jurek", 100}, {"tobi", 99}, {"lennart", 77}};
}
