...
void SnakeGeometry::initSphereGeometry(int latitudeBands, int longitudeBands) {
...
void SnakeGeometry::drawSphereGeometry(QOpenGLShaderProgram *program, QMatrix4x4 projection) {
    program->setUniformValue("mvp_matrix", projection * modelMatrix);
    ...
    if (child != nullptr) {
        child->drawSphereGeometry(program, projection);
    }
